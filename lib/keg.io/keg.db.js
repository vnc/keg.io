...
//
// TODO: This should return whether or not the INSERT actually 
//       affected any rows.  This can be done using the 
//       db.prepare and statement.step functionality of node-sqlite
KegDb.prototype.insertCoaster = function(rfid, coaster_id) {   
	var self = this;
	this.db.execute("INSERT INTO user_coaster(rfid, coaster_id, earned_date) " +
					"SELECT ?, ?, ? WHERE NOT EXISTS (SELECT 1 FROM " +
					"user_coaster WHERE rfid = ? and coaster_id = ?)"
					,[rfid, coaster_id, self.formatDate(new Date()), rfid, coaster_id]		
					,function(error, rows) {
					 	if (error) { self.logger.error("SQL Error:" + error); } 
					});
};
...
KegDb.prototype.updateCoasters = function(rfid, volumeInOunces) {
	/* 
	   Current Coasters:                                                                             
