					...
					if ((rows != null) && (rows.length > 0)){                               
						...
						kegDb.getUserCoasters(d, function(rows) {
						   if ((rows != null) && (rows.length > 0)) {
								...
								// TODO: Remove/turn down the verbosity on this logging
								self.logger.info("COASTERS!:")
								for(var i = 0; i < rows.length; i++)
								{
									self.logger.info(rows[i].coaster_id + "|" + rows[i].name + "|"+ rows[i].description );
								}
							}
						});
					}
