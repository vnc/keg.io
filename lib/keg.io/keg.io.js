...
{                        
	var frequencyInMs = 1000;
	console.log("flowsLeft: " + flowsLeft);         
	var self = this;             
...
{                        
	...
	var randomUser = Math.floor(Math.random() * 3); // between 0-2
	var userRfid = "";
	switch(randomUser)
	{
		case 0:       
	...
	setTimeout(function() {     
		...
		self.hashEmail("dcarney@gmail.com", function(hash) {
			    self.emit("pour", "Drinky McDrinkerson|" + hash);
              ...
              case 'tag':
				...
				kegDb.validateUser(d, function(rows) {
					if ((rows != null) && (rows.length > 0)){ 
						...
						self._allowPour();	
						self.emit("pour", rows[0].first_name + " " + rows[0].last_name + "|" + rows[1].email);
					}
					else 
					{
						console.log("Denied.");
						// For some reason, we have to pass some data on the emit() call
						if (eventData != '0000000000') self.emit("deny", "dummydata");
					}
				});
                break;
