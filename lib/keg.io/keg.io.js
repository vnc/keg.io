...
Keg.prototype.init = function(logger, dev, dbName, twitterEnabled) {       
	                    
	this.logger = logger;
	kegDb.init(logger, dbName);
	...
	{                       
		this.logger.info('Using DEVMODE....');                
		this.fakePour(); 
		this.fakeTemp();  
		return;
	}
	...
	f.addListener('error', function(fd) {
		this.logger.error('Serial port read error. Check that device is valid \'' + dev + '\'');
			...
			switch(this.actions[i].toLowerCase())
              ...
              case 'tag':
				...
				kegDb.validateUser(d, function(rows) {
					...
					{
						this.logger.info("Denied an RFID.");
