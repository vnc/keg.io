...
Keg.prototype.init = function(logger, 
	...
	self.logger.info("****DATA FROM SERIAL PORT EVENT.");
	...
	// DEBUGGING JUNK
	this.arduino.addListener('drain', function() { self.logger.info("****DRAIN EVENT FROM WRITE STREAM") });
	this.arduino.addListener('close', function() { self.logger.info("****CLOSE EVENT FROM WRITE STREAM") });
	this.arduino.addListener('pipe', function(src) { self.logger.info("****PIPE EVENT FROM WRITE STREAM") });
	// END DEBUGGING JUNK
};
...
Keg.prototype._onData = function(data) {
	...
	for (var i = 0; i < this.actions.length; i++) {
		if (data.indexOf(this.actions[i]) >= 0) {
			...
			var eventName = this.actions[i].toUpperCase();
			var eventData = d.toUpperCase();
