...
Keg.prototype.init = function(logger, 
	...
	this.websocketPort = port; // assign serial port to this keg object
	var self = this; // initialize self so 'this' is accessible in events below

	// listen for kegerators via websocket and then setup listeners for error and data
	var socket = io.listen(parseInt(this.websocketPort));

	socket.on('connection', function(client){
		self._lookupKegerator(client, function(kegeratorId) {
			if (! kegeratorId ) {
				client.disconnect(); // refuse connection from unknown kegerator
				self.logger.info('Unknown kegerator client connection refused.');
			}

			logger.info('Kegerator client connected');

			client.on('data', function(data) {
				var nickname = client.get('nickname');
				if (nickname) {
...
Keg.prototype._lookupKegerator = function(client, callback) {
	...
	client.on('data', function(data) {
		if (!this.isValidAuthMessage(data)) return; // invalid auth message

		kegDb.validateKegerator(data, function(rows) {
